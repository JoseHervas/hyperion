...
class BM_external_confidence(LogicAdapter):
    """
    Derived from chatterbot.logic.BestMatch

    A logic adapter that returns a response based on known responses to
    the closest matches to the input statement.

    This logic adapter takes the minimum_similarity_threshold from the 
    config/bot.json file.
    """
    ...
    def process(self, input_statement, additional_response_selection_parameters=None):
        ...
        for result in search_results:
            ...
            if result.confidence >= self.minimum_similarity_threshold:
